...
export class CreateUserComponent {
    ...
    generateForm(formData:any){
      this.apiSer.getGeneratedUserFields(formData).subscribe({
          next: (resObj: any) => {
            if(resObj) {       
    ...
    dataSave(event:any) {
      ...
      updateFormValue(event) {
        ...
        this.apiSer.updateUserData(event).subscribe({
          next: (resObj: any) => {
            if(resObj) {
              this.snackBarToastr.openSnackBar(resObj?.message || constants.genericSystemMsg.update, false);
              this.router.navigate([ROUTE_URL.adminUser.default]); 
            }
          },
          error: (data:any) => {
            const { error } = data || {};
       ...
       //Create Api Cal
     saveFormValue(event) {
      this.apiSer.saveUserData(event).subscribe({
        next: (resObj: any) => {
          if(resObj) {       
