...
export class StatusComponent {
    ...
    tableColumn = [
      { header: 'Status Name', columnDef: 'status' },
...
constructor(
  ...
  ngOnInit() {
    ...
    //Get Hire Data
    fetchApiData() {
        this.apiSer.getStatusData().subscribe({
          next: (response) => {
      ...
      // Handle Edit action
      onEdit(row: any): void {
        this.router.navigate([ROUTE_URL.adminStatus.update,row._id]);
      }
      ...
      onDelete(row: any): void {
        ...
        deleteItem(deleteData: any): void {
          ...
          this.apiSer.deleteStatusData(deleteDataId).subscribe({
            next: (resObj: any) => {
              if(resObj) {       
  ...
  onButtonClick(event: Event) {  
    this.router.navigate([ROUTE_URL.adminStatus.create]); 
  }
}
