...
export class CreateStatusComponent {
    ...
    generateForm(formData:any){
      this.apiSer.getGeneratedStatusFields(formData).subscribe({
          next: (resObj: any) => {
            if(resObj) {       
    ...
    dataSave(event:any) {
      ...
      updateFormValue(event) {
        ...
        this.apiSer.updateStatusData(event).subscribe({
          next: (resObj: any) => {
            if(resObj) {
              this.snackBarToastr.openSnackBar(resObj?.message || constants.genericSystemMsg.update, false);
              this.router.navigate([ROUTE_URL.adminStatus.default]); 
            }
          },
          error: (data:any) => {
            const { error } = data || {};
       ...
       //Create Api Cal
     saveFormValue(event) {
      this.apiSer.saveAdminStatusData(event).subscribe({
        next: (resObj: any) => {
          if(resObj) {       
