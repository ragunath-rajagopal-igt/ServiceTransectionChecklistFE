...
export class ItemComponent {
    ...
    tableColumn = [
      { header: 'Item', columnDef: 'itemActivity' },
...
constructor(
  ...
  ngOnInit() {
    ...
    //Get Hire Data
    fetchApiData() {
        this.apiSer.getItemData().subscribe({
          next: (response) => {
      ...
      // Handle Edit action
      onEdit(row: any): void {
        this.router.navigate([ROUTE_URL.adminItem.update,row._id]);
      }
      ...
      onDelete(row: any): void {
        ...
        deleteItem(deleteData: any): void {
          ...
          this.apiSer.deleteItemData(deleteDataId).subscribe({
            next: (resObj: any) => {
              if(resObj) {       
  ...
  onButtonClick(event: Event) {  
    this.router.navigate([ROUTE_URL.adminItem.create]); 
  }
}
