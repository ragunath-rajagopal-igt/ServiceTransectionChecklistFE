...
export class UserComponent {
    ...
    tableColumn = [
      { header: 'User Name', columnDef: 'username' },
      { header: 'Email', columnDef: 'email' },
      // { header: 'Role', columnDef: 'role', field:{type:'options',options:constants.role}},
    ];
...
constructor(
  ...
  ngOnInit() {
    ...
    //Get Hire Data
    fetchApiData() {
        this.apiSer.getUserData().subscribe({
          next: (response) => {
      ...
      // Handle Edit action
      onEdit(row: any): void {
        this.router.navigate([ROUTE_URL.adminUser.update,row._id]);
      }
      ...
      onDelete(row: any): void {
        ...
        deleteItem(deleteData: any): void {
          ...
          this.apiSer.deleteUserData(deleteDataId).subscribe({
            next: (resObj: any) => {
              if(resObj) {       
  ...
  onButtonClick(event: Event) {  
    this.router.navigate([ROUTE_URL.adminUser.create]); 
  }
}
