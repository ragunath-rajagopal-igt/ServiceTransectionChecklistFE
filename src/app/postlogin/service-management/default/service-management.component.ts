...
export class ServiceManagementComponent {
  ...
  ngOnInit() {
    this.fetchServiceManagementData();
  }

  //Get Hire Data
  fetchServiceManagementData() {
    this.apiSer.getServiceManagementData().subscribe({
      next: (response) => {
  ...
  onButtonClick(event: Event) {  
    this.router.navigate([ROUTE_URL.serviceManagement.create]); 
  }

  // Handle Edit action
  onEdit(row: any): void {
    this.router.navigate([ROUTE_URL.serviceManagement.update,row._id]);
  }
  ...
  deleteItem(deleteData: any): void {
    ...
    this.apiSer.deleteServiceManagementData(deleteDataId).subscribe({
      next: (resObj: any) => {
        if(resObj) {       
