...
constructor(
  ...
  ngOnInit() {
    ...
    //Get Hire Data
    fetchApiData() {
        this.apiSer.getProductData().subscribe({
          next: (response) => {
      ...
      // Handle Edit action
      onEdit(row: any): void {
        this.router.navigate([ROUTE_URL.adminProductName.update,row._id]);
      }
      ...
      onDelete(row: any): void {
        ...
        deleteItem(deleteData: any): void {
          ...
          this.apiSer.deleteProductNameData(deleteDataId).subscribe({
            next: (resObj: any) => {
              if(resObj) {       
  ...
  onButtonClick(event: Event) {  
    this.router.navigate([ROUTE_URL.adminProductName.create]); 
  }
}
